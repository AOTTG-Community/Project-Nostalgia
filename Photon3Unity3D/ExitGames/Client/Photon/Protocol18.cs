    ...
    public class Protocol18 : IProtocol
    {
        ...
        private object Read(StreamBuffer stream, byte gpType)
        {
            ...
            switch (gpType)
            {
                ...
                case 26:
                    return this.DeserializeEventData(stream);
        ...
        private object ReadCustomTypeArray(StreamBuffer stream)
        {
            ...
            if (num > 0)
            {
                for (int i = 0; i < num; i++)
                {
                    ...
                    if (length > 16)
                    {
                        ...
                        for (; i < num; i++)
                        {
                            stream.Position += (int)ReadCompressedUInt32(stream);
                        }
                        return false;
                    }
            ...
            if (!Protocol.CodeDict.TryGetValue(b, out CustomType customType))
            {
                //Message(Response)
                return false;
            }
            Array result = Array.CreateInstance(customType.Type, num);
            if (num == 0)
                return result;
            int index = 0;
            foreach (byte[] array in data)
            {
                if (customType.DeserializeFunction(array, out object val))
                {
        ...
        private IDictionary[] ReadDictionaryArray(StreamBuffer stream)
        {
            ...
            Type type = this.ReadDictionaryType(stream, out keyReadType, out valueReadType);
            uint num = this.ReadCompressedUInt32(stream);
            IDictionary[] array = (IDictionary[])Array.CreateInstance(type, num);
            for (int i = 0; i < num; i++)
            {
        ...
        private Dictionary<byte, object> ReadParameterTable(StreamBuffer stream)
        {
            ...
            for (int i = 0; i < num; i++)
            {
                ...
                dictionary[key] = this.Read(stream, (byte)stream.ReadByte());
            }
            return dictionary;
        }
        ...
        internal void WriteCompressedInt32(StreamBuffer stream, int value, bool writeType)
        {
            if (writeType)
            {
                if (value == 0)
        ...
        {
            byte[] bufferAndAdvance = stream.GetBufferAndAdvance(2, out int num);
        ...
        internal void WriteDouble(StreamBuffer stream, double value, bool writeType)
        {
            ...
            try
            {
                ...
                Buffer.BlockCopy(this.memDoubleBlock, 0, bufferAndAdvance, num, 8);
            }
            finally
            {
                Monitor.Exit(obj);
        ...
        internal void WriteSingle(StreamBuffer stream, float value, bool writeType)
        {
            ...
            try
            {
                ...
                Buffer.BlockCopy(this.memFloatBlock, 0, bufferAndAdvance, num, 4);
            }
            finally
            {
                Monitor.Exit(obj);
        ...
        public static int DecodeZig32(uint value)
        {
            return (int)(value >> 1 ^ 0L - (value & 1));
        }

        public static int ReadInt12(byte[] buff, int position, bool negative = false)
        {
            if (negative)
            {
                return -ReadUshortStatic(buff, position);
            }
            return ReadUshortStatic(buff, position);
        }

        public static int ReadInteger(byte[] buffer, int position)
        {
            uint num = 0u;
            int offset = 0;
            while (offset != 35)
            {
                ...
                num = (uint)((int)num | (b & 0x7F) << offset);
                offset += 7;
                if ((b & 0x80) == 0)
                {
                    break;
            ...
            return DecodeZig32(num);
        }

        public static ushort ReadUshortStatic(byte[] buff, int position)
        {
            return (ushort)(buff[position + 1] | buff[position] << 8);
        }
        ...
        public override EventData DeserializeEventData(StreamBuffer din)
        {
            ...
            eventData.Parameters = this.ReadParameterTable(din);
            return eventData;
        }
        ...
        public override OperationRequest DeserializeOperationRequest(StreamBuffer din)
        {
            ...
            operationRequest.Parameters = this.ReadParameterTable(din);
        ...
        public override OperationResponse DeserializeOperationResponse(StreamBuffer stream)
        {
            ...
            operationResponse.Parameters = this.ReadParameterTable(stream);
        ...
        {
            byte b = ((gpType != 0) ? ((byte)(gpType - 128)) : ((byte)stream.ReadByte()));
            int num = (int)ReadCompressedUInt32(stream);
            byte[] array = new byte[num];
