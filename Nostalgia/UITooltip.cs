    ...
    private void SetText(string tooltipText)
    {
        if ((this.text != null) && !string.IsNullOrEmpty(tooltipText))
        {
            ...
            if (this.uiCamera != null)
            {
                ...
                Vector2 vector3 = new Vector2((num2 * this.mSize.x) / ((float) Screen.width), (num2 * this.mSize.y) / ((float) Screen.height));
                this.mPos.x = Mathf.Min(this.mPos.x, 1f - vector3.x);
                this.mPos.y = Mathf.Max(this.mPos.y, vector3.y);
