...
public class UIDrawCall : MonoBehaviour
{
    ...
    private void UpdateMaterials()
    {
        if (this.mClipping != Clipping.None)
        {
            ...
            if (this.mClipping != Clipping.None)
            {
                string str3 = this.mSharedMat.shader.name.Replace(" (AlphaClip)", string.Empty).Replace(" (SoftClip)", string.Empty);
                if ((this.mClipping == Clipping.HardClip) || (this.mClipping == Clipping.AlphaClip))
                {
                    shader = Shader.Find(str3 + " (AlphaClip)");
                }
                else if (this.mClipping == Clipping.SoftClip)
                {
                    shader = Shader.Find(str3 + " (SoftClip)");
                }
                if (shader == null)
                {
