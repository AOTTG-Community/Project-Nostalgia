...
public class TITAN_EREN : Photon.MonoBehaviour
{
    ...
    public void Update()
    {
        ...
        if (!this.isHit)
        {
            ...
            {
                if (FengCustomInputs.Main.isInputDown[InputCode.attack0] || FengCustomInputs.Main.isInputDown[InputCode.attack1])
                {
                    bool flag = false;
                    if ((IN_GAME_MAIN_CAMERA.CameraMode == CameraType.WOW && FengCustomInputs.Main.isInput[InputCode.down]) || FengCustomInputs.Main.isInputDown[InputCode.attack1])
                    {
                        if (IN_GAME_MAIN_CAMERA.CameraMode == CameraType.WOW && FengCustomInputs.Main.isInputDown[InputCode.attack1] && FengCustomInputs.Main.inputKey[11] == KeyCode.Mouse1)
                        {
                            flag = true;
                        }
                        if (flag)
                ...
                if (FengCustomInputs.Main.isInputDown[InputCode.salute])
                {
            ...
            if (!this.isAttack)
            {
                if ((this.grounded || base.animation.IsPlaying("idle")) && !base.animation.IsPlaying("jump_start") && !base.animation.IsPlaying("jump_air") && !base.animation.IsPlaying("jump_land") && FengCustomInputs.Main.isInput[InputCode.bothRope])
                {
            ...
            {
                if (base.animation[this.attackAnimation].time >= 0.1f && FengCustomInputs.Main.isInputDown[InputCode.attack0])
                {
