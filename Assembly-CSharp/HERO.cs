...
public class HERO : Optimization.Caching.Bases.HeroBase
{
    ...
    private void bombInit()
    {
        ...
        if (Bomb.BombModeEnabled)
        {
            int rad = Bomb.MyBombRad;
            int range = Bomb.MyBombRange;
            int speed = Bomb.MyBombSpeed;
            int cd = Bomb.MyBombCD;
            ...
            this.skillCDDuration = ((FengGameManagerMKII.FGM.RoundTime > 10f) ? 10f : 5f);
        }
    }

    private void bombUpdate()
    {
        if (FengCustomInputs.Main.isInputDown[InputCode.attack1] && this.skillCDDuration <= 0f)
        {
            ...
            hash.Add("RCBombR", color.r);
            hash.Add("RCBombG", color.g);
            hash.Add("RCBombB", color.b);
            hash.Add("RCBombA", color.a);
            PhotonNetwork.player.SetCustomProperties(hash);
            this.detonate = false;
        ...
        if (this.myBomb != null && !this.myBomb.disabled)
        {
            ...
            bool flag2 = false;
            if (FengCustomInputs.Main.isInputUp[InputCode.attack1])
            {
                this.detonate = true;
            }
            else if (FengCustomInputs.Main.isInputDown[InputCode.attack1] && this.detonate)
            {
                this.detonate = false;
                flag2 = true;
            }
    ...
    private void checkDashDoubleTap()
    {
        ...
        if (FengCustomInputs.Main.isInputDown[InputCode.up])
        {
            if (this.uTapTime == -1f)
            {
                this.uTapTime = 0f;
        ...
        if (FengCustomInputs.Main.isInputDown[InputCode.down])
        {
            if (this.dTapTime == -1f)
            {
                this.dTapTime = 0f;
        ...
        if (FengCustomInputs.Main.isInputDown[InputCode.left])
        {
            if (this.lTapTime == -1f)
            {
                this.lTapTime = 0f;
        ...
        if (FengCustomInputs.Main.isInputDown[InputCode.right])
        {
            if (this.rTapTime == -1f)
            {
                this.rTapTime = 0f;
    ...
    private void FixedUpdate()
    {
        ...
        if (this.isLaunchLeft)
        {
            if (this.bulletLeft != null && bulletLeft.isHooked())
            {
                ...
                if (Vector3.Angle(baseR.velocity, vector3) > 90f && FengCustomInputs.Main.isInput[InputCode.jump])
                {
                    flag2 = true;
                    flag = true;
                }
        ...
        if (this.isLaunchRight)
        {
            if (this.bulletRight != null && bulletRight.isHooked())
            {
                ...
                if (Vector3.Angle(baseR.velocity, vector4) > 90f && FengCustomInputs.Main.isInput[InputCode.jump])
                {
                    flag3 = true;
                    flag = true;
                }
        ...
        else
        {
            ...
            else if (this.State == HeroState.Idle && this.isPressDirectionTowardsHero(num, num2) && !FengCustomInputs.Main.isInput[InputCode.jump] && !FengCustomInputs.Main.isInput[InputCode.leftRope] && !FengCustomInputs.Main.isInput[InputCode.rightRope] && !FengCustomInputs.Main.isInput[InputCode.bothRope] && this.IsFrontGrounded() && !baseA.IsPlaying("wallrun") && !baseA.IsPlaying("dodge"))
            {
            ...
            else if (!baseA.IsPlaying("attack5") && !baseA.IsPlaying("special_petra") && !baseA.IsPlaying("dash") && !baseA.IsPlaying("jump"))
            {
                ...
                if (!flag2 && !flag3 && !this.isMounted && FengCustomInputs.Main.isInput[InputCode.jump] && this.currentGas > 0f)
                {
                    if (num != 0f || num2 != 0f)
                    {
        ...
        if (flag2 || flag3)
        {
            ...
            float num13 = Input.GetAxis("Mouse ScrollWheel") * 5555f;
            float idk = 1.53938f * (1f + Mathf.Clamp(num13, -0.8f, 0.8f));
        ...
        {
            IN_GAME_MAIN_CAMERA.MainCamera.GetComponent<Camera>().fieldOfView = Mathf.Lerp(IN_GAME_MAIN_CAMERA.MainCamera.GetComponent<Camera>().fieldOfView, Mathf.Min(100f, this.currentSpeed + 40f), 0.1f);
        }
        else
        {
        ...
        {
            baseR.rotation = Quaternion.Lerp(baseGT.rotation, this.targetRotation, Time.deltaTime * 6f);
        }
    }
    ...
    private void netDie2(int viewID = -1, string titanName = "")
    {
        ...
        if (IN_GAME_MAIN_CAMERA.GameType == GameType.Multi && BasePV.IsMine)
        {
            ...
            if (viewID != -1)
            {
                ...
                if (photonView != null)
                {
                    FengGameManagerMKII.FGM.SendKillInfo(true, (string)photonView.owner.Properties[PhotonPlayerProperty.name], false, (string)PhotonNetwork.player.Properties[PhotonPlayerProperty.name], 0);
            ...
            {
                FengGameManagerMKII.FGM.SendKillInfo(true, titanName, false, (string)PhotonNetwork.player.Properties[PhotonPlayerProperty.name], 0);
            }
        ...
        if (IN_GAME_MAIN_CAMERA.GameType != GameType.Single)
        {
            Optimization.Caching.Pool.NetworkEnable("hitMeat2", baseT.position, Quaternion.Euler(270f, 0f, 0f), 0);
        }
        else
        {
            Pool.Enable("hitMeat2", baseT.position, Quaternion.identity);//gameObject = (GameObject)UnityEngine.Object.Instantiate(CacheResources.Load("hitMeat2"));
        }
        if (BasePV.IsMine)
        {
    ...
    private void OnDestroy()
    {
        ...
        {
            DestroyImmediate(this.setup.part_cape);
        }
        if (this.setup.part_hair_1 != null)
        {
            DestroyImmediate(this.setup.part_hair_1);
        }
        if (this.setup.part_hair_2 != null)
        {
            DestroyImmediate(this.setup.part_hair_2);
        }
    }
    ...
    private void ShowGas()
    {
        ...
        CacheGameObject.Find("gasL1").GetComponent<UISprite>().fillAmount = CacheGameObject.Find("gasR1").GetComponent<UISprite>().fillAmount = num;
        if (num <= 0f)
        {
            SpriteCache.Find("gasL").color = SpriteCache.Find("gasR").color = Colors.red;
            return;
        }
        if (num <= 0.1f)
        {
            SpriteCache.Find("gasL").color = SpriteCache.Find("gasR").color = Colors.orange;
            return;
        }
        if (num < 0.3f)
        {
            SpriteCache.Find("gasL").color = SpriteCache.Find("gasR").color = Colors.yellow;
            return;
        }
        SpriteCache.Find("gasL").color = Colors.white;
        SpriteCache.Find("gasR").color = Colors.white;
    }
    ...
    private void Start()
    {
        ...
        if (FengGameManagerMKII.Level.HorsesEnabled && IN_GAME_MAIN_CAMERA.GameType == GameType.Multi && BasePV.IsMine)
        {
        ...
        //if (IsLocal)
        //{
        //    enables.Add(wLeft.name);
        //    enables.Add(wRight.name);
        //}
        if (!IsLocal)
        {
        ...
        if (IN_GAME_MAIN_CAMERA.GameType == GameType.Multi)
        {
            ...
            render.material = Labels.tahoma.material;
            //txt.transform.localScale = new Vector3(4.9f, 4.9f);
            txt.font = Labels.tahoma;
            txt.fontSize = 20;
            ...
            if ((int)BasePV.owner.Properties[PhotonPlayerProperty.team] == 2)
            {
                this.myNetWorkName.GetComponent<TextMesh>().text = "[FF0000]AHSS\n[FFFFFF]";
            }
            string text = (string)BasePV.owner.Properties[PhotonPlayerProperty.guildName];
            if (text != string.Empty)
            {
                TextMesh component = this.myNetWorkName.GetComponent<TextMesh>();
                ...
                component.text = string.Concat(new string[]
                {
                    ...
                    text,
                    "\n[FFFFFF]",
                    (string)BasePV.owner.Properties[PhotonPlayerProperty.name]
                });
            }
            else
            {
                TextMesh component2 = this.myNetWorkName.GetComponent<TextMesh>();
                component2.text += (string)BasePV.owner.Properties[PhotonPlayerProperty.name];
            }
            myNetWorkName.GetComponent<TextMesh>().text = myNetWorkName.GetComponent<TextMesh>().text.ToRGBA();
        }
        if (IsLocal)
        {
            ShowGas();
            ...
            if (IN_GAME_MAIN_CAMERA.GameType == GameType.Multi)
            {
                ...
                {
                    PlayerPrefs.GetFloat("cameraDistance") + 0.3f
                });
                bombImmune = true;
    ...
    private void suicide()
    {
        ...
        this.netDie(baseR.velocity * 50f, false, -1, string.Empty, true);
    ...
    public void lateUpdate()
    {
        ...
        if (IN_GAME_MAIN_CAMERA.cameraTilt == 1 && (IN_GAME_MAIN_CAMERA.GameType == GameType.Single || BasePV.IsMine))
        {
            ...
            IN_GAME_MAIN_CAMERA.BaseCamera.transform.rotation = Quaternion.Lerp(IN_GAME_MAIN_CAMERA.BaseCamera.transform.rotation, to2, Time.deltaTime * 2f);
        }
    ...
    public void netDie(Vector3 v, bool isBite, int viewID = -1, string titanName = "", bool killByTitan = true)
    {
        ...
        if (BasePV.IsMine)
        {
            ...
            if (viewID != -1)
            {
                ...
                if (photonView != null)
                {
                    FengGameManagerMKII.FGM.SendKillInfo(killByTitan, (string)photonView.owner.Properties[PhotonPlayerProperty.name], false, (string)PhotonNetwork.player.Properties[PhotonPlayerProperty.name], 0);
                    photonView.owner.SetCustomProperties(new Hashtable
                    {
                        {
                            PhotonPlayerProperty.kills,
            ...
            {
                FengGameManagerMKII.FGM.SendKillInfo(!(titanName == string.Empty), titanName, false, (string)PhotonNetwork.player.Properties[PhotonPlayerProperty.name], 0);
            }
        }
        if (BasePV.IsMine)
        {
    ...
    {
        this.skillCD = CacheGameObject.Find("skill_cd_" + this.skillID);
    ...
    public void setTeam(int team)
    {
        ...
        if (IN_GAME_MAIN_CAMERA.GameType == GameType.Multi && BasePV.IsMine)
        {
            BasePV.RPC("setMyTeam", PhotonTargets.OthersBuffered, new object[]
            {
                team
            });
            PhotonNetwork.player.SetCustomProperties(new Hashtable
    ...
    public void update()
    {
        ...
        if (this.State == HeroState.Grab && !this.Gunner)
        {
            if (this.skillID == "jean")
            {
                if (this.State != HeroState.Attack && (FengCustomInputs.Main.isInputDown[InputCode.attack0] || FengCustomInputs.Main.isInputDown[InputCode.attack1]) && this.escapeTimes > 0 && !baseA.IsPlaying("grabbed_jean"))
                {
        ...
        {
            if (FengCustomInputs.Main.isInputDown[InputCode.jump] && !baseA.IsPlaying("jump") && !baseA.IsPlaying("horse_geton"))
            {
                this.idle();
                this.crossFade("jump", 0.1f);
                this.sparks.enableEmission = false;
            }
            if (FengCustomInputs.Main.isInputDown[InputCode.dodge] && !baseA.IsPlaying("jump") && !baseA.IsPlaying("horse_geton"))
            {
                this.dodge(false);
                return;
            }
        ...
        switch (_state)
        {
            case HeroState.Idle:
                ...
                if (FengCustomInputs.Main.isInputDown[InputCode.flare2])
                {
                    this.shootFlare(2);
                }
                if (FengCustomInputs.Main.isInputDown[InputCode.flare3])
                {
                    this.shootFlare(3);
                }
                ...
                if (this.myHorse != null && this.isMounted && FengCustomInputs.Main.isInputDown[InputCode.dodge])
                {
                    this.getOffHorse();
                }
                if ((baseA.IsPlaying(this.standAnimation) || !this.grounded) && FengCustomInputs.Main.isInputDown[InputCode.reload])
                {
                    this.changeBlade();
                    if (Gunner) ShowBullets();
                    else ShowBlades();
                ...
                if (baseA.IsPlaying(this.standAnimation) && FengCustomInputs.Main.isInputDown[InputCode.salute])
                {
                    this.salute();
                    return;
                }
                if (!this.isMounted && (FengCustomInputs.Main.isInputDown[InputCode.attack0] || FengCustomInputs.Main.isInputDown[InputCode.attack1]) && !this.Gunner)
                {
                    bool flag2 = false;
                    if (FengCustomInputs.Main.isInputDown[InputCode.attack1])
                    {
                    ...
                    else if (FengCustomInputs.Main.isInputDown[InputCode.attack0])
                    {
                        if (this.needLean)
                        {
                            if (FengCustomInputs.Main.isInput[InputCode.left])
                        ...
                        else if (FengCustomInputs.Main.isInput[InputCode.left])
                        {
                            this.attackAnimation = "attack2";
                        }
                        else if (FengCustomInputs.Main.isInput[InputCode.right])
                ...
                if (this.Gunner)
                {
                    if (FengCustomInputs.Main.isInput[InputCode.attack1])
                    {
                        this.leftArmAim = true;
                        this.rightArmAim = true;
                    }
                    else if (FengCustomInputs.Main.isInput[InputCode.attack0])
                    {
                    ...
                    if (flag5 || FengCustomInputs.Main.isInputUp[InputCode.attack0])
                    {
                        if (this.grounded)
                        {
                            if (this.leftGunHasBullet && this.rightGunHasBullet)
            ...
            case HeroState.Attack:
                if (!this.Gunner)
                {
                    if (!FengCustomInputs.Main.isInput[InputCode.attack0])
                    {
                        this.buttonAttackRelease = true;
                    }
                    if (!this.attackReleased)
        ...
        if (!baseA.IsPlaying("attack3_1") && !baseA.IsPlaying("attack5") && !baseA.IsPlaying("special_petra") && State != HeroState.Grab)
        {
            ...
            if (FengCustomInputs.Main.isInput[InputCode.bothRope])
            {
                this.QHold = true;
                this.EHold = true;
